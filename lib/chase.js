...
var set_slider_val = function(val) {
    set_cp(Math.floor(Math.pow(2, val/1000)));
};

//var set_cp_from_dom = function(s) {
    //var dec = base_to_dec(s);
    //if (isNaN(dec)) {
        //return;
    //}
    //set_cp(dec);
//};
//
// base from/to should be a string (hex, etc.)
//var convert_base = function(s, base_from, base_to) {
    //var dec = parseInt(s, cp_base_number[base_from]);
    //if (isNaN(dec)) {
        //return null;
    //}
    //return dec.toString(cp_base_number[base_to]);
//}
...
$(function() {
    ...
    $('#amount').change(function () {
        var dec = base_to_dec($(this).val());
        var slider_val = cp_to_slider_val(dec);
