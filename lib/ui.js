...
var set_slider_val = function(val) {
    var converted = Math.floor(Math.pow(2, (val)/RESOLUTION_MULTIPLIER)) - 1;
    set_cp(converted);
    set_href(converted);
};
...
var set_href = function(cp) {
    var url = location.href;
    window.history.replaceState("", "", url.replace(/cp=(\d)*/, function() {
        return "cp=" + cp;
    }));
}
